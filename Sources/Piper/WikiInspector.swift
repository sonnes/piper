                        ...
                        if let resource = source["resource"] as? String, let url = URL(string: resource) {
                            Button { model.openLink(url, from: document) } label: {
                                Label(source["title"] as? String ?? resource, systemImage: "arrow.up.right")
                                    .font(PiperTheme.ui(12)).lineLimit(3).multilineTextAlignment(.leading)
    ...
    private var info: some View {
        VStack(alignment: .leading, spacing: 16) {
            VStack(alignment: .leading, spacing: 8) {
                ...
                if document.isStale { Label("Review overdue", systemImage: "clock").font(PiperTheme.ui(11)).foregroundStyle(PiperTheme.warning).padding(.horizontal, 8) }
